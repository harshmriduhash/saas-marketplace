...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const order = await razorpay.orders.create({
      ...
      currency: 'INR',
      receipt,
      notes: { listingId: String(listingId), purchaseType: purchaseType ?? 'featured' },
    });

    return res.status(200).json({ orderId: order.id, amount: order.amount, key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID ?? key_id });
  } catch (err: any) {
